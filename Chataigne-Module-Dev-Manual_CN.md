...
function now() {
    return new Date() - 0;  // JUCE 安全的时间戳，返回毫秒
}
```
...
**问题:** JUCE 的 Date 对象在 JS 引擎中不支持 `getTime()` 或 `valueOf()`。

**解决方法:** `var timestamp = new Date() - 0;` 返回自 epoch 起的毫秒数。
